    ...
    <style>
        ...
        * {
        ...
        .nvr {
            padding: 18px;
            display: grid;
            grid-template-columns: minmax(200px, 1.3fr) 155px minmax(220px, 1fr);
            gap: 20px;
            align-items: center
        }
        ...
        .camera-thumbnail:focus-visible {
            border-color: var(--warn);
            outline: 0
        }

        .camera-thumbnail img,
        .camera-thumbnail .no-image {
            display: block;
            width: 100%;
            height: 92px;
            object-fit: cover
...
<body>
    <main>
        <header>
            <div>
                <h1>Monitoramento de NVRs</h1>
                <p>Status de conectividade dos gravadores e cameras.</p>
            </div>
        ...
        <?php else: ?>
            ...
            <section class="nvr-list" aria-label="Lista de NVRs">
                <?php foreach ($nvrRows as $nvr): ?>
                    <article class="nvr">
                        <div>
                            <div class="nvr-name"><?= $escape($nvr['name']) ?></div>
                            <div class="ip"><?= $escape($nvr['ip']) ?></div>
                        </div>
                        <div class="status <?= $nvr['online'] ? 'online' : '' ?>"><i class="dot"></i><?= $nvr['online'] ? 'Respondendo' : 'Sem resposta' ?></div>
                        <div>
                            <div class="camera-count"><?= $nvr['active'] ?> <small>de <?= $nvr['total'] ?> Cameras ativas</small></div><?php if (!$nvr['offline']): ?><div class="none">Todas as cameras respondendo</div><?php endif; ?>
                        </div>
                        <?php if ($nvr['offline']): ?>
    ...
    <script>
        const modal = document.getElementById('image-modal'),
            preview = document.getElementById('image-modal-preview'),
            title = document.getElementById('image-modal-title'),
            danrley = document.getElementById('send-danrley'),
            gabriel = document.getElementById('send-gabriel');
        const whatsappLink = (phone, message) => `https://wa.me/${phone}?text=${encodeURIComponent(message)}`;
        document.querySelectorAll('.camera-thumbnail[data-image]').forEach((thumbnail) => thumbnail.addEventListener('click', () => {
            const ip = thumbnail.dataset.ip,
                imageUrl = new URL(thumbnail.dataset.image, window.location.href);
